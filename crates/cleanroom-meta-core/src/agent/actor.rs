...
use crate::agent::{MetaAgentBuilder, MetaDeriveT, MetaExecutor, MetaBaseAgent, MetaHookOutcome};
...
pub struct MetaActorAgent {}

impl MetaAgentType for MetaActorAgent {
...
/// `MetaTask`s to subscribed `Topic<MetaTask>`.
#[cfg(not(target_arch = "wasm32"))]
#[derive(Clone)]
pub struct MetaActorAgentHandle<T: MetaDeriveT + MetaExecutor + MetaHooks + Send + Sync> {
    pub agent: Arc<MetaBaseAgent<T, MetaActorAgent>>,
    pub actor_ref: ActorRef<MetaTask>,
}

#[cfg(not(target_arch = "wasm32"))]
impl<T: MetaDeriveT + MetaExecutor + MetaHooks> MetaActorAgentHandle<T> {
    /// Get the actor reference (`ActorRef<MetaTask>`) for direct messaging.
    pub fn addr(&self) -> ActorRef<MetaTask> {
        self.actor_ref.clone()
    }

    /// Get a clone of the agent reference for querying metadata or invoking
    /// methods that require `Arc<MetaBaseAgent<..>>`.
    pub fn agent(&self) -> Arc<MetaBaseAgent<T, MetaActorAgent>> {
        self.agent.clone()
    }
}

#[cfg(not(target_arch = "wasm32"))]
impl<T: MetaDeriveT + MetaExecutor + MetaHooks> Debug for MetaActorAgentHandle<T> {
...
#[cfg(not(target_arch = "wasm32"))]
#[derive(Debug)]
pub struct AgentActor<T: MetaDeriveT + MetaExecutor + MetaHooks>(
    pub Arc<MetaBaseAgent<T, MetaActorAgent>>,
);

#[cfg(not(target_arch = "wasm32"))]
impl<T: MetaDeriveT + MetaExecutor + MetaHooks> AgentActor<T> {}

#[cfg(not(target_arch = "wasm32"))]
impl<T: MetaDeriveT + MetaExecutor + MetaHooks> MetaAgentBuilder<T, MetaActorAgent>
where
    T: Send + Sync + 'static,
    serde_json::Value: From<<T as MetaExecutor>::Output>,
    <T as MetaDeriveT>::Output: From<<T as MetaExecutor>::Output>,
    <T as MetaExecutor>::Error: Into<RunnableAgentError>,
{
    /// Build the MetaBaseAgent and return a wrapper that includes the actor reference
    pub async fn build(self) -> Result<MetaActorAgentHandle<T>, Error> {
        ...
        let agent: Arc<MetaBaseAgent<T, MetaActorAgent>> = Arc::new(
            MetaBaseAgent::<T, MetaActorAgent>::new(self.inner, llm, self.memory, tx, self.stream).await?,
        );
        ...
        Ok(MetaActorAgentHandle { agent, actor_ref })
    }

    pub fn subscribe(mut self, topic: Topic<MetaTask>) -> Self {
...
#[cfg(not(target_arch = "wasm32"))]
impl<T: MetaDeriveT + MetaExecutor + MetaHooks> MetaBaseAgent<T, MetaActorAgent> {
    ...
    pub async fn run(
        self: Arc<Self>,
        task: MetaTask,
    ) -> Result<<T as MetaDeriveT>::Output, RunnableAgentError>
    where
        Value: From<<T as MetaExecutor>::Output>,
        <T as MetaDeriveT>::Output: From<<T as MetaExecutor>::Output>,
        <T as MetaExecutor>::Error: Into<RunnableAgentError>,
    {
    ...
    pub async fn run_stream(
        ...
        std::pin::Pin<
            Box<dyn Stream<Item = Result<<T as MetaDeriveT>::Output, RunnableAgentError>> + Send>,
        >,
        RunnableAgentError,
    >
    where
...
#[cfg(not(target_arch = "wasm32"))]
#[async_trait]
impl<T: MetaDeriveT + MetaExecutor + MetaHooks> Actor for AgentActor<T>
where
    T: Send + Sync + 'static,
    serde_json::Value: From<<T as MetaExecutor>::Output>,
    <T as MetaDeriveT>::Output: From<<T as MetaExecutor>::Output>,
...
mod tests {
    ...
    impl Runtime for TestRuntime {
        fn id(&self) -> cleanroom_meta_protocol::RuntimeID {
            cleanroom_meta_protocol::RuntimeID::new_v4()
        }
    ...
    async fn test_actor_builder_requires_llm() {
        ...
        let err = MetaAgentBuilder::<_, MetaActorAgent>::new(mock)
            .runtime(runtime)
            .build()
            .await
            .unwrap_err();
    ...
    async fn test_actor_builder_subscribes_topics() {
        ...
        let llm = Arc::new(MockLLMProvider);
        let runtime = Arc::new(TestRuntime::new());
        let topic = Topic::<MetaTask>::new("jobs");

        let _handle = MetaAgentBuilder::<_, MetaActorAgent>::new(mock)
            .llm(llm)
    ...
    async fn test_actor_agent_tx_missing_returns_error() {
        ...
        let mut agent = MetaBaseAgent::<_, MetaActorAgent>::new(mock, llm, None, tx, false)
            .await
            .unwrap();
        agent.tx = None;

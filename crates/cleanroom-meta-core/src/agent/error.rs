...
pub enum RunnableAgentError {
    ...
    #[error("MetaTask processing failed: {0}")]
    TaskError(String),
...
impl From<crate::agent::executor::turn_engine::TurnEngineError> for RunnableAgentError {
    fn from(error: crate::agent::executor::turn_engine::TurnEngineError) -> Self {
        match error {
            crate::agent::executor::turn_engine::TurnEngineError::MetaError(err) => {
                RunnableAgentError::MetaError(err)
            }
...
impl From<crate::agent::prebuilt::executor::MetaBasicExecutorError> for RunnableAgentError {
    fn from(error: crate::agent::prebuilt::executor::MetaBasicExecutorError) -> Self {
        match error {
            crate::agent::prebuilt::executor::MetaBasicExecutorError::MetaError(err) => {
                RunnableAgentError::MetaError(err)
            }
            crate::agent::prebuilt::executor::MetaBasicExecutorError::Other(err) => {
                RunnableAgentError::ExecutorError(err)
            }
        }
    }
...
impl From<crate::agent::prebuilt::executor::ReActExecutorError> for RunnableAgentError {
    fn from(error: crate::agent::prebuilt::executor::ReActExecutorError) -> Self {
        match error {
            crate::agent::prebuilt::executor::ReActExecutorError::MetaError(err) => {
                RunnableAgentError::MetaError(err)
            }
...
mod tests {
    use super::*;
    use crate::agent::prebuilt::executor::{MetaBasicExecutorError, ReActExecutorError};
    ...
    fn test_runnable_agent_error_display() {
        ...
        let error = RunnableAgentError::TaskError("MetaTask failed".to_string());
        assert_eq!(error.to_string(), "MetaTask processing failed: MetaTask failed");
    ...
    fn test_agent_result_error_display() {
        let error = MetaResultError::NoOutput;
        assert_eq!(error.to_string(), "No output available in result");

        let error = MetaResultError::AgentOutputError("Custom output error".to_string());
        assert_eq!(
            error.to_string(),
            "Agent output extraction error: Custom output error"
        );

        let error = MetaResultError::agent_output_error("Helper constructor error");
        assert!(matches!(error, MetaResultError::AgentOutputError(_)));
    }
    ...
    fn test_agent_result_error_from_json_error() {
        ...
        if let Err(json_err) = json_error {
            let agent_error: MetaResultError = json_err.into();
            assert!(matches!(
                agent_error,
                MetaResultError::DeserializationError(_)
            ));
        }
    }
    ...
    fn test_from_llm_error_preserves_typed_llm_error_direct() {
        let source = MetaError::GuardrailBlocked {
        ...
        assert!(matches!(
            converted,
            RunnableAgentError::MetaError(MetaError::GuardrailBlocked { .. })
        ));
    }

    #[test]
    fn test_from_basic_executor_preserves_typed_llm_error() {
        let wrapped = MetaBasicExecutorError::from(MetaError::GuardrailBlocked {
            phase: GuardrailPhase::Input,
            guard: "prompt-injection".to_string().into(),
            rule_id: "prompt_injection_detected".to_string().into(),
            category: "prompt_injection".to_string().into(),
    ...
    fn test_from_react_executor_preserves_typed_llm_error() {
        let wrapped = ReActExecutorError::from(MetaError::GuardrailBlocked {
            phase: GuardrailPhase::Input,
            guard: "prompt-injection".to_string().into(),
            rule_id: "prompt_injection_detected".to_string().into(),
            category: "prompt_injection".to_string().into(),

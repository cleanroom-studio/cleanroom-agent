...
use cleanroom_meta_llm::{chat::MetaMessage, error::MetaError};
use regex::Regex;
...
mod tests {
    use super::*;
    use cleanroom_meta_llm::chat::{MetaMessage, MetaRole, MessageType};
    ...
    impl MockMemoryProvider {
        ...
        fn with_messages(messages: Vec<MetaMessage>) -> Self {
            Self {
                messages,
                should_fail: false,
            }
    ...
    impl MemoryProvider for MockMemoryProvider {
        async fn remember(&mut self, message: &MetaMessage) -> Result<(), MetaError> {
            if self.should_fail {
                return Err(MetaError::ProviderError("Mock memory error".to_string()));
            }
        ...
        async fn recall(
            ...
            _query: &str,
            limit: Option<usize>,
        ) -> Result<Vec<MetaMessage>, MetaError> {
            if self.should_fail {
                return Err(MetaError::ProviderError("Mock recall error".to_string()));
            }
            let limit = limit.unwrap_or(self.messages.len());
            Ok(self.messages.iter().take(limit).cloned().collect())
        }

        async fn clear(&mut self) -> Result<(), MetaError> {
            if self.should_fail {
                return Err(MetaError::ProviderError("Mock clear error".to_string()));
            }
            self.messages.clear();
            Ok(())
        }
    ...
    fn test_message_condition_eq() {
        ...
        let different_message = MetaMessage {
            role: MetaRole::User,
            message_type: MessageType::Text,
    ...
    fn test_message_condition_contains() {
        ...
        let non_matching_message = MetaMessage {
            role: MetaRole::User,
            message_type: MessageType::Text,
    ...
    fn test_message_condition_custom() {
        ...
        let goodbye_message = MetaMessage {
            role: MetaRole::User,
            message_type: MessageType::Text,
    ...
    fn test_message_condition_empty() {
        ...
        let non_empty_message = MetaMessage {
            role: MetaRole::User,
            message_type: MessageType::Text,
    ...
    fn test_message_condition_all() {
        ...
        let matching_message = MetaMessage {
            role: MetaRole::User,
            message_type: MessageType::Text,
            content: "this is a test message".to_string(),
        };
    ...
    fn test_message_condition_regex() {
        ...
        let number_message = MetaMessage {
            role: MetaRole::User,
            message_type: MessageType::Text,
        ...
        let no_number_message = MetaMessage {
            role: MetaRole::User,
            message_type: MessageType::Text,
...
pub enum MessageCondition {
    ...
    Custom(Arc<dyn Fn(&MetaMessage) -> bool + Send + Sync>),
    /// Empty
    Empty,
...
pub trait MemoryProvider: Send + Sync {
    ...
    /// * `Err(MetaError)` if storage failed
    async fn remember(&mut self, message: &MetaMessage) -> Result<(), MetaError>;
    ...
    /// # Returns
    ///
    /// * `Ok(Vec<MetaMessage>)` containing relevant messages
    /// * `Err(MetaError)` if retrieval failed
    ...
    /// * `Err(MetaError)` if clearing failed
    async fn clear(&mut self) -> Result<(), MetaError>;

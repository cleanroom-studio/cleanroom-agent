...
impl SlidingWindowMemory {
    ...
    pub fn messages(&self) -> Vec<MetaMessage> {
    ...
    pub fn recent_messages(&self, limit: usize) -> Vec<MetaMessage> {
    ...
    pub fn replace_with_summary(&mut self, summary: String) {
        self.messages.clear();
        self.messages
            .push_back(MetaMessage::assistant().content(summary).build());
...
impl MemoryProvider for SlidingWindowMemory {
    ...
    fn preload(&mut self, data: Vec<MetaMessage>) -> bool {
        self.messages.clear();
        for msg in data {
...
mod tests {
    ...
    fn test_replace_with_summary() {
        ...
        assert_eq!(messages[0].role, MetaRole::Assistant);
    }

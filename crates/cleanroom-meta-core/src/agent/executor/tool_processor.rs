...
use cleanroom_meta_protocol::{ActorID, Event, SubmissionId};
...
use crate::agent::{MetaHooks, Context, MetaHookOutcome};
#[cfg(target_arch = "wasm32")]
use futures::SinkExt;
...
impl ToolProcessor {
    ...
    pub(crate) async fn process_single_tool_call_with_hooks<H: MetaHooks>(
        hooks: &H,
        context: &Context,
    ...
    async fn execute_tool(tool: &dyn MetaToolT, tool_name: &str, tool_args: &str) -> ToolCallResult {
...
mod tests {
    use super::*;
    use crate::agent::{MetaDeriveT, Context, MetaHookOutcome};
    use crate::tests::MockLLMProvider;
    use crate::tool::{ToolCallError, ToolRuntime, MetaToolT};
    ...
    async fn test_process_tool_calls_valid_tool() {
        let tools: Vec<Box<dyn MetaToolT>> = vec![Box::new(MockTool::new("tool_a"))];
        let calls = vec![make_tool_call("1", "tool_a", r#"{"x":1}"#)];
        let ctx = ToolCallContext::new(
            cleanroom_meta_protocol::SubmissionId::new_v4(),
            cleanroom_meta_protocol::ActorID::new_v4(),
        );
    ...
    async fn test_process_single_tool_call_failure() {
        let tools: Vec<Box<dyn MetaToolT>> = vec![Box::new(MockTool::failing("fail_tool"))];
    ...
    async fn test_send_event_with_none_tx() {
        ...
        ToolProcessor::send_event(
            &None,
            cleanroom_meta_protocol::Event::StreamComplete {
                sub_id: cleanroom_meta_protocol::SubmissionId::new_v4(),
            },
        )
        .await;
    }
    ...
    #[async_trait]
    impl MetaDeriveT for AbortHooks {
        type Output = String;

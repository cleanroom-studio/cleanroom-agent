use crate::agent::config::MetaConfig;
use crate::agent::memory::MemoryProvider;
use crate::agent::{MetaExecutor, Context, output::MetaOutputT};
use crate::tool::{MetaToolT, to_llm_tool};
use async_trait::async_trait;
use cleanroom_meta_llm::MetaLlm;
use cleanroom_meta_llm::chat::Tool;
use cleanroom_meta_protocol::{ActorID, Event};
...
#[async_trait]
pub trait MetaDeriveT: Send + Sync + 'static + Debug {
    /// The output type this agent produces
    type Output: MetaOutputT;
...
pub trait MetaAgentType: 'static + Send + Sync {
    fn type_name() -> &'static str;
}

/// Base agent type that wraps an MetaDeriveT implementation with additional runtime components
#[derive(Clone)]
pub struct MetaBaseAgent<T: MetaDeriveT + MetaExecutor + MetaHooks + Send + Sync, A: MetaAgentType> {
    /// The inner agent implementation (from macro)
    pub(crate) inner: Arc<T>,
    /// LLM provider for this agent
    pub(crate) llm: Arc<dyn MetaLlm>,
    /// Agent ID
    pub id: ActorID,
...
impl<T: MetaDeriveT + MetaExecutor + MetaHooks, A: MetaAgentType> Debug for MetaBaseAgent<T, A> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(format!("A: {} - T: {}", self.inner().name(), A::type_name()).as_str())
    }
}

impl<T: MetaDeriveT + MetaExecutor + MetaHooks, A: MetaAgentType> MetaBaseAgent<T, A> {
    /// Create a new MetaBaseAgent wrapping an MetaDeriveT implementation
    pub async fn new(
        inner: T,
        llm: Arc<dyn MetaLlm>,
    ...
    pub fn tools(&self) -> Vec<Box<dyn MetaToolT>> {
        self.inner.tools()
    }
    ...
    pub fn agent_config(&self) -> MetaConfig {
...
fn tools_match_cached(tools: &[Box<dyn MetaToolT>], cached: &[Tool]) -> bool {
...
mod tests {
    ...
    use crate::agent::{MetaConfig, MetaDirectAgent};
    ...
    async fn test_base_agent_creation_with_memory_and_stream() {
        ...
        let base_agent = MetaBaseAgent::<_, MetaDirectAgent>::new(mock_agent, llm, Some(memory), tx, true)
            .await
            .unwrap();
    ...
    async fn test_base_agent_create_context_populates_config() {
        ...
        let llm = Arc::new(MockLLMProvider);
        let (tx, _): (Sender<Event>, Receiver<Event>) = channel(32);
        let base_agent = MetaBaseAgent::<_, MetaDirectAgent>::new(mock_agent, llm, None, tx, false)
            .await
            .unwrap();

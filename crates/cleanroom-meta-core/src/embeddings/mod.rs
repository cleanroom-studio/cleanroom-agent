...
pub type SharedEmbeddingProvider = Arc<dyn MetaEmbeddingProvider + Send + Sync>;
// NOTE: upstream used `Arc<[f32]>` but `Arc<[T]>` doesn't implement
// `serde::Deserialize`. `Vec<f32>` is API-compatible (`.as_ref()`, `.clone()`,
// `From<Vec<f32>>` all work the same way for downstream code) and serde-friendly.
pub type VecArc = Vec<f32>;
...
impl distance::VectorDistance for Embedding {
    fn cosine_similarity(&self, other: &Self, normalize: bool) -> f32 {
        self.vec
            .as_slice()
            .cosine_similarity(other.vec.as_slice(), normalize)
    }
}

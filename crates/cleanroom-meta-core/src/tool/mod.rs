use cleanroom_meta_llm::chat::{MetaFunctionTool, Tool};
pub use cleanroom_meta_protocol::ToolCallResult;
...
pub trait MetaToolT: Send + Sync + Debug + ToolRuntime {
...
/// A wrapper that allows Arc<dyn MetaToolT> to be used as Box<dyn MetaToolT>
/// This is useful for sharing tools across multiple agents without cloning
/// Wrapper around `Arc<dyn MetaToolT>` that presents a `Box<dyn MetaToolT>`-like API.
/// Useful when sharing tool instances across multiple agents without cloning.
#[derive(Debug)]
pub struct SharedTool {
    inner: Arc<dyn MetaToolT>,
}

impl SharedTool {
    /// Create a new SharedTool from an Arc<dyn MetaToolT>
    pub fn new(tool: Arc<dyn MetaToolT>) -> Self {
        Self { inner: tool }
    }
}
...
/// Helper function to convert Vec<Arc<dyn MetaToolT>> to Vec<Box<dyn MetaToolT>>
/// This is useful when implementing MetaDeriveT::tools() with shared tools
/// Convert a vector of `Arc<dyn MetaToolT>` into boxed trait objects for use in
/// agent definitions.
pub fn shared_tools_to_boxes(tools: &[Arc<dyn MetaToolT>]) -> Vec<Box<dyn MetaToolT>> {
    tools
        .iter()
        .map(|t| Box::new(SharedTool::new(Arc::clone(t))) as Box<dyn MetaToolT>)
        .collect()
}

/// Convert a MetaToolT trait object to an LLM Tool
#[allow(clippy::borrowed_box)]
pub fn to_llm_tool(tool: &Box<dyn MetaToolT>) -> Tool {
    Tool {
...
mod tests {
    use super::*;
    use cleanroom_meta_llm::chat::Tool;
    ...
    fn test_boxed_tool_to_tool_conversion() {
        ...
        let boxed_tool: Box<dyn MetaToolT> = Box::new(mock_tool);
    ...
    fn test_tool_trait_object_usage() {
        let tools: Vec<Box<dyn MetaToolT>> = vec![
    ...
    fn test_tool_trait_object_send_sync() {
        fn assert_send_sync<T: Send + Sync>() {}
        assert_send_sync::<Box<dyn MetaToolT>>();
    }

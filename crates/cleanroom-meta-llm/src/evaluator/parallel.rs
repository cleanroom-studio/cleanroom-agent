...
pub struct ParallelEvaluator {
    ...
    providers: Vec<(String, Box<dyn MetaLlm>)>,
...
impl ParallelEvaluator {
    ...
    pub fn new(providers: Vec<(String, Box<dyn MetaLlm>)>) -> Self {
        Self {
            providers,
            scoring_fns: Vec::new(),
            include_timing: true,
    ...
    ) -> Result<Vec<ParallelEvalResult>, MetaError> {
        let futures = self
            .providers
            .iter()
            .map(|(id, provider)| {

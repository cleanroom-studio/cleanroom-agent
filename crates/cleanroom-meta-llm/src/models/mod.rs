use crate::{builder::MetaBackend, error::MetaError};
...
/// Standard model entry structure used by OpenAiProvider-compatible providers
...
/// Standard model list response structure used by OpenAiProvider-compatible providers
#[derive(Clone, Debug)]
...
mod tests {
    ...
    struct MockModelListResponse {
        ...
        /// Backend identifier — plain string, no coupling to MetaBackend enum.
        backend: String,
    }
    ...
    #[async_trait]
    impl MetaModelsProvider for MockModelsProvider {
        async fn list_models(
            &self,
            _request: Option<&ModelListRequest>,
        ) -> Result<Box<dyn ModelListResponse>, MetaError> {
    ...
    #[async_trait]
    impl MetaModelsProvider for DefaultModelsProvider {}
    ...
    fn test_backend_variants() {
        ...
        for (backend, expected) in cases {
            // StandardModelListResponse stores MetaBackend internally and returns String
    ...
    async fn test_models_provider_error_handling() {
        ...
        let result = provider.list_models(None).await;
        match result {
            Err(MetaError::ProviderError(msg)) => {

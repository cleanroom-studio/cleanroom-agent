...
use crate::{
    ...
    chat::{
        ...
        StreamResponse, MetaStructuredOutputFormat, Tool,
    },
...
type BoxStream<T> = Pin<Box<dyn Stream<Item = Result<T, MetaError>> + Send>>;
...
///
/// ```rust,ignore
/// use cleanroom_meta_llm::pipeline::PipelineBuilder;
/// use cleanroom_meta_llm::optim::{CacheLayer, CacheConfig};
...
// ---------------------------------------------------------------------------
// Materialised (cloneable) chat response — avoids storing `dyn MetaResponse`
// ---------------------------------------------------------------------------
...
impl MetaResponse for CachedChatResponse {
...
struct TeeStream<T> {
    inner: Pin<Box<dyn Stream<Item = Result<T, MetaError>> + Send>>,
    buffer: Vec<T>,
    cache: Cache<Vec<T>>,
    key: u64,
    ttl: Option<Duration>,
...
impl InMemoryCacheLayer {
    fn new(inner: Arc<dyn MetaLlm>, config: CacheConfig) -> Self {
        Self {
            inner,
...
fn hash_chat_user_prompt(
    ...
    let prompt = messages
        .iter()
        .rev()
        .find(|message| message.role == MetaRole::User)
...
fn hash_completion(req: &MetaCompletionRequest, json_schema: Option<&MetaStructuredOutputFormat>) -> u64 {
    let mut h = DefaultHasher::default();
...
// ---------------------------------------------------------------------------
// MetaProvider — non-streaming cached; streaming via TeeStream
// ---------------------------------------------------------------------------

#[async_trait]
impl MetaProvider for InMemoryCacheLayer {
    async fn chat(
        &self,
        messages: &[MetaMessage],
        json_schema: Option<MetaStructuredOutputFormat>,
...
// ---------------------------------------------------------------------------
// MetaCompletionProvider — optionally cached
// ---------------------------------------------------------------------------

#[async_trait]
impl MetaCompletionProvider for InMemoryCacheLayer {
    async fn complete(
        &self,
            ...
            match self.completion_cache.read() {
                Ok(cache) => {
                    if let Some(entry) = cache.get(&key)
                        && !is_expired(&self.config, entry.created_at)
                    {
                        return Ok(MetaCompletionResponse {
...
// ---------------------------------------------------------------------------
// MetaEmbeddingProvider — optionally cached
// ---------------------------------------------------------------------------

#[async_trait]
impl MetaEmbeddingProvider for InMemoryCacheLayer {
...
// ---------------------------------------------------------------------------
// MetaModelsProvider — always delegate (model lists are not cached)
// ---------------------------------------------------------------------------

#[async_trait]
impl MetaModelsProvider for InMemoryCacheLayer {
...
impl MetaLlm for InMemoryCacheLayer {}
...
mod tests {
    use super::*;
    use crate::{
        MetaHasConfig, MetaLlm, MetaNoConfig,
        chat::{
            MetaMessage, MetaProvider, MetaResponse, MetaRole, MessageType, StreamChoice,
            StreamChunk, StreamDelta, StreamResponse, MetaStructuredOutputFormat, Tool,
        },
        completion::{MetaCompletionProvider, MetaCompletionRequest, MetaCompletionResponse},
        embedding::MetaEmbeddingProvider,
        error::MetaError,
        models::MetaModelsProvider,
        optim::CacheLayer,
    ...
    impl MetaResponse for MockChatResp {
        fn text(&self) -> Option<String> {
            Some(self.0.clone())
        }
    ...
    #[async_trait]
    impl MetaProvider for MockInner {
        async fn chat(
            &self,
        ...
        ) -> Result<Pin<Box<dyn Stream<Item = Result<String, MetaError>> + Send>>, MetaError>
        {
            ...
            let chunks: Vec<Result<String, MetaError>> = vec![
                Ok("chunk1".into()),
                Ok("chunk2".into()),
                Ok("chunk3".into()),
            ];
        ...
        ) -> Result<Pin<Box<dyn Stream<Item = Result<StreamChunk, MetaError>> + Send>>, MetaError>
        {
            ...
            let chunks: Vec<Result<StreamChunk, MetaError>> = vec![
    ...
    #[async_trait]
    impl MetaCompletionProvider for MockInner {
        async fn complete(
            &self,
    ...
    #[async_trait]
    impl MetaEmbeddingProvider for MockInner {
    ...
    #[async_trait]
    impl MetaModelsProvider for MockInner {}

    impl MetaLlm for MockInner {}

    impl MetaHasConfig for MockInner {
        type Config = MetaNoConfig;
    }
    ...
    fn make_pipeline(config: CacheConfig) -> (Arc<MockInner>, Arc<dyn MetaLlm>) {
        let inner = MockInner::new();
        let provider = PipelineBuilder::new(inner.clone() as Arc<dyn MetaLlm>)
    ...
    async fn collect_stream<T: Clone>(
        stream: Pin<Box<dyn Stream<Item = Result<T, MetaError>> + Send>>,
    ) -> Vec<T> {
        let mut out = Vec::new();
        let mut s = stream;
    ...
    async fn test_chat_cache_hit_does_not_call_inner() {
        ...
        let messages = vec![MetaMessage::user().content("hello").build()];
    ...
    async fn test_chat_cache_different_messages_are_separate_entries() {
        let (inner, provider) = make_pipeline(CacheConfig::default());
        let msg_a = vec![MetaMessage::user().content("question A").build()];
        let msg_b = vec![MetaMessage::user().content("question B").build()];
    ...
    async fn test_chat_cache_reuses_same_prompt_across_different_history() {
        let (inner, provider) = make_pipeline(CacheConfig::default());
        let msg_a = vec![MetaMessage::user().content("repeat prompt").build()];
        let msg_b = vec![
            MetaMessage::assistant().content("older context").build(),
            MetaMessage::user().content("repeat prompt").build(),
        ];
    ...
    async fn test_chat_cache_ttl_expiry() {
        ...
        let messages = vec![MetaMessage::user().content("ttl test").build()];
    ...
    async fn test_chat_cache_max_size_evicts_oldest() {
        ...
        let msg_a = vec![MetaMessage::user().content("A").build()];
        let msg_b = vec![MetaMessage::user().content("B").build()];
        let msg_c = vec![MetaMessage::user().content("C").build()];
    ...
    async fn test_chat_stream_first_call_hits_inner() {
        let (inner, provider) = make_pipeline(CacheConfig::default());
        let messages = vec![MetaMessage::user().content("stream test").build()];
    ...
    async fn test_chat_stream_cache_disabled_always_calls_inner() {
        ...
        let messages = vec![MetaMessage::user().content("no cache").build()];
    ...
    async fn test_chat_stream_ttl_expiry() {
        ...
        let messages = vec![MetaMessage::user().content("ttl stream").build()];
    ...
    async fn test_chat_stream_max_size_evicts_oldest() {
        ...
        let msg_a = vec![MetaMessage::user().content("SA").build()];
        let msg_b = vec![MetaMessage::user().content("SB").build()];
        let msg_c = vec![MetaMessage::user().content("SC").build()];
    ...
    async fn test_chat_stream_with_tools_first_call_hits_inner() {
        let (inner, provider) = make_pipeline(CacheConfig::default());
        let messages = vec![MetaMessage::user().content("tool stream").build()];
    ...
    async fn test_chat_stream_with_tools_cache_hit() {
        let (inner, provider) = make_pipeline(CacheConfig::default());
        let messages = vec![MetaMessage::user().content("tool stream cached").build()];
    ...
    async fn test_chat_stream_with_tools_disabled() {
        ...
        let messages = vec![MetaMessage::user().content("no cache tools").build()];
    ...
    async fn test_chat_stream_struct_first_call_hits_inner() {
        let (inner, provider) = make_pipeline(CacheConfig::default());
        let messages = vec![MetaMessage::user().content("struct stream").build()];
    ...
    async fn test_chat_stream_struct_cache_hit() {
        let (inner, provider) = make_pipeline(CacheConfig::default());
        let messages = vec![MetaMessage::user().content("struct cached").build()];
    ...
    async fn test_chat_stream_struct_disabled() {
        ...
        let messages = vec![MetaMessage::user().content("no cache struct").build()];
    ...
    async fn test_chat_stream_error_not_cached() {
        ...
        #[async_trait]
        impl MetaProvider for ErrorOnFirst {
            async fn chat_with_tools(
                &self,
                _: &[MetaMessage],
                _: Option<&[Tool]>,
                _: Option<MetaStructuredOutputFormat>,
            ) -> Result<Box<dyn MetaResponse>, MetaError> {
                Err(MetaError::Generic("not impl".into()))
            }
            async fn chat_stream(
                &self,
                _: &[MetaMessage],
            ...
            ) -> Result<Pin<Box<dyn Stream<Item = Result<String, MetaError>> + Send>>, MetaError>
            {
                ...
                if !self.failed.swap(true, Ordering::SeqCst) {
                    ...
                    let s: Vec<Result<String, MetaError>> = vec![
                        Ok("ok".into()),
                        Err(MetaError::Generic("mid-stream error".into())),
                    ];
                    Ok(Box::pin(futures::stream::iter(s)))
                } else {
                    // Second call: clean stream
                    let s: Vec<Result<String, MetaError>> =
        ...
        #[async_trait]
        impl MetaCompletionProvider for ErrorOnFirst {
            async fn complete(
                &self,
                _: &MetaCompletionRequest,
                _: Option<MetaStructuredOutputFormat>,
            ) -> Result<MetaCompletionResponse, MetaError> {
                Err(MetaError::Generic("not impl".into()))
            }
        ...
        #[async_trait]
        impl MetaEmbeddingProvider for ErrorOnFirst {
            async fn embed(&self, _: Vec<String>) -> Result<Vec<Vec<f32>>, MetaError> {
                Err(MetaError::Generic("not impl".into()))
            }
        }
        #[async_trait]
        impl MetaModelsProvider for ErrorOnFirst {}
        impl MetaLlm for ErrorOnFirst {}
        ...
        let provider = PipelineBuilder::new(inner as Arc<dyn MetaLlm>)
            .add_layer(CacheLayer::with_defaults())
            .build();

        let messages = vec![MetaMessage::user().content("error test").build()];
    ...
    async fn test_completion_cache_ttl_expiry() {
        ...
        let req = MetaCompletionRequest::new("ttl-prompt");
    ...
    async fn test_all_caches_are_independent() {
        ...
        let messages = vec![MetaMessage::user().content("x").build()];
        let req = MetaCompletionRequest::new("x");

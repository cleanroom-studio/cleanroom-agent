//! AnthropicProvider API client implementation for chat and completion functionality.
//!
//! This module provides integration with AnthropicProvider's Claude models through their API.

use std::{collections::HashMap, sync::Arc};

use crate::{
    FunctionCall, ToolCall,
    builder::{MetaBackend, MetaBuilder},
    chat::{
        MetaMessage, MetaProvider, MetaResponse, MetaRole, MessageType, StreamChunk,
        MetaStructuredOutputFormat, Tool, MetaToolChoice, Usage,
    },
    completion::{MetaCompletionProvider, MetaCompletionRequest, MetaCompletionResponse},
    embedding::MetaEmbeddingProvider,
    error::MetaError,
    models::{ModelListRawEntry, ModelListRequest, ModelListResponse, MetaModelsProvider},
};
...
/// Client for interacting with AnthropicProvider's API.
///
/// Provides methods for chat and completion requests using AnthropicProvider's models.
#[derive(Debug)]
...
/// AnthropicProvider-specific tool format that matches their API structure
...
/// Request payload for AnthropicProvider's messages API endpoint.
#[derive(Serialize, Debug)]
...
/// Individual message in an AnthropicProvider chat conversation.
#[derive(Serialize, Debug)]
...
/// Response from AnthropicProvider's messages API endpoint.
...
/// Usage information from AnthropicProvider API response.
#[derive(Deserialize, Debug)]
...
/// Content block within an AnthropicProvider API response.
...
/// Response from AnthropicProvider's streaming messages API endpoint.
...
/// Content block within an AnthropicProvider streaming content_block_start event.
#[derive(Deserialize, Debug)]
...
/// Delta content within an AnthropicProvider streaming response.
#[derive(Deserialize, Debug)]
...
impl MetaResponse for AnthropicCompleteResponse {
...
impl AnthropicProvider {
    /// Converts a slice of MetaMessage into AnthropicProvider's message format.
    ///
    /// This helper method handles all message types including text, images, PDFs,
    /// tool use, and tool results.
    fn convert_messages_to_anthropic<'a>(messages: &'a [MetaMessage]) -> Vec<AnthropicMessage<'a>> {
        messages
            .iter()
            .filter(|m| m.role != MetaRole::System)
            .map(|m| AnthropicMessage {
                role: match m.role {
                    MetaRole::User => "user",
                    MetaRole::Assistant => "assistant",
                    MetaRole::System => unreachable!("system messages are filtered before mapping"),
                    MetaRole::Tool => "user",
                },
    ...
    /// Prepares AnthropicProvider tools and tool_choice from the provided tools and instance configuration.
    ///
    ...
    fn prepare_tools_and_choice<'a>(
        ...
        let tool_choice_map = match tool_choice {
            Some(MetaToolChoice::Auto) => {
                Some(HashMap::from([("type".to_string(), "auto".to_string())]))
            }
            Some(MetaToolChoice::Any) => Some(HashMap::from([("type".to_string(), "any".to_string())])),
            Some(MetaToolChoice::Tool(tool_name)) => Some(HashMap::from([
                ("type".to_string(), "tool".to_string()),
                ("name".to_string(), tool_name.clone()),
            ])),
            Some(MetaToolChoice::None) => {
    ...
    /// Creates a new AnthropicProvider client with the specified configuration.
    ///
    /// # Arguments
    ///
    /// * `api_key` - AnthropicProvider API key for authentication
...
#[async_trait]
impl MetaProvider for AnthropicProvider {
    /// Sends a chat request to AnthropicProvider's API.
    ///
    /// # Arguments
    ///
    ...
    async fn chat_with_tools(
        ...
        if self.api_key.is_empty() {
            return Err(MetaError::AuthError("Missing AnthropicProvider API key".to_string()));
        }
        ...
        let system_message = messages
            .iter()
            .find(|msg| msg.role == MetaRole::System)
        ...
        {
            log::trace!("AnthropicProvider request payload: {}", json);
        }

        log::debug!("AnthropicProvider request: POST /v1/messages");
        let resp = request.send().await?;
        log::debug!("AnthropicProvider HTTP status: {}", resp.status());

        let resp = resp.error_for_status()?;

        let body = resp.text().await?;
        let json_resp: AnthropicCompleteResponse = serde_json::from_str(&body)
            .map_err(|e| MetaError::HttpError(format!("Failed to parse JSON: {e}")))?;
    ...
    /// Sends a streaming chat request to AnthropicProvider's API.
    ///
    /// # Arguments
    ///
    /// * `messages` - Slice of chat messages representing the conversation
    ...
    ) -> Result<std::pin::Pin<Box<dyn Stream<Item = Result<String, MetaError>> + Send>>, MetaError>
    {
        ...
        if !response.status().is_success() {
            ...
            return Err(MetaError::ResponseFormatError {
                message: format!("AnthropicProvider API returned error status: {status}"),
    ...
    ) -> Result<std::pin::Pin<Box<dyn Stream<Item = Result<StreamChunk, MetaError>> + Send>>, MetaError>
    {
        ...
        if log::log_enabled!(log::Level::Trace)
            && let Ok(json) = serde_json::to_string(&req_body)
        {
            log::trace!("AnthropicProvider streaming request payload: {}", json);
        }

        log::debug!("AnthropicProvider request: POST /v1/messages (streaming with tools)");
        let response = request.send().await?;
        log::debug!("AnthropicProvider HTTP status: {}", response.status());
...
/// Creates an SSE stream that parses AnthropicProvider tool use events into StreamChunk.
fn create_anthropic_tool_stream(
    response: reqwest::Response,
) -> std::pin::Pin<Box<dyn Stream<Item = Result<StreamChunk, MetaError>> + Send>> {
...
#[async_trait]
impl MetaCompletionProvider for AnthropicProvider {
    /// Sends a completion request to AnthropicProvider's API.
    ///
...
#[async_trait]
impl MetaEmbeddingProvider for AnthropicProvider {
...
impl ModelListResponse for AnthropicModelListResponse {
    ...
    fn get_backend(&self) -> String {
        MetaBackend::Anthropic.to_string()
    }
}
...
#[async_trait]
impl MetaModelsProvider for AnthropicProvider {
...
impl crate::MetaLlm for AnthropicProvider {}

impl crate::MetaHasConfig for AnthropicProvider {
    type Config = crate::MetaNoConfig;
}

/// Parses a Server-Sent Events (SSE) chunk from AnthropicProvider's streaming API.
///
/// # Arguments
///
...
/// * `Err(MetaError)` - If parsing fails
fn parse_anthropic_sse_chunk(chunk: &str) -> Result<Option<String>, MetaError> {
...
/// Parses AnthropicProvider SSE chunks with tool use support.
///
/// This parser handles all AnthropicProvider streaming event types including:
...
) -> Result<Option<StreamChunk>, MetaError> {
    for line in chunk.lines() {
        let line = line.trim();
        if let Some(data) = line.strip_prefix("data: ") {
            match serde_json::from_str::<AnthropicStreamResponse>(data) {
                        ...
                        "content_block_stop" => {
                            if let Some(index) = response.index {
                                ...
                                if let Some(state) = tool_states.remove(&index) {
                                    // AnthropicProvider API requires tool_use.input to be a JSON object.
...
impl MetaBuilder<AnthropicProvider> {
    pub fn build(self) -> Result<Arc<AnthropicProvider>, MetaError> {
        let api_key = self.api_key.ok_or_else(|| {
            MetaError::InvalidRequest("No API key provided for AnthropicProvider".to_string())
        })?;

        let anthro = AnthropicProvider::new(
            api_key,
            self.model,
            self.max_tokens,
            self.temperature,
...
mod tests {
    use super::*;
    use crate::chat::{MetaFunctionTool, ImageMime};
...
"#;
    ...
    fn test_parse_stream_tool_use_complete_empty_arguments() {
        ...
        // Empty string arguments cause AnthropicProvider API to reject with:
...
"#;
        ...
        match result {
            Some(StreamChunk::ToolUseComplete { index, tool_call }) => {
                ...
                // CRITICAL: arguments must be "{}" not "" for AnthropicProvider API compatibility
                assert_eq!(
...
"#;
    ...
    fn test_convert_messages_to_anthropic_tool_result() {
        ...
        let converted = AnthropicProvider::convert_messages_to_anthropic(&messages);
    ...
    fn test_prepare_tools_and_choice_auto() {
        ...
        let tools_list = [tool];
        let (tools, choice) =
            AnthropicProvider::prepare_tools_and_choice(Some(&tools_list), &Some(MetaToolChoice::Auto));
    ...
    fn test_prepare_tools_and_choice_specific_tool() {
        ...
        let tools_list = [tool];
        let (tools, choice) = AnthropicProvider::prepare_tools_and_choice(
            Some(&tools_list),
            &Some(MetaToolChoice::Tool("lookup".to_string())),
        );
        assert!(tools.is_some());
    ...
    fn test_prepare_tools_and_choice_ignored_without_tools() {
        let (tools, choice) = AnthropicProvider::prepare_tools_and_choice(None, &Some(MetaToolChoice::Auto));
    ...
    fn test_anthropic_builder_requires_api_key() {
        let err = MetaBuilder::<AnthropicProvider>::new().build().unwrap_err();

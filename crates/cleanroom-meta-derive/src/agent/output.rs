...
pub(crate) struct MetaOutputParser {
    output_data: StructuredOutputFormat,
    ident: Option<Ident>,
}

impl MetaOutputParser {
    pub fn parse(&mut self, input: TokenStream) -> TokenStream {
        ...
        let expanded = quote! {
            impl cleanroom_meta::core::agent::MetaOutputT for #struct_ident {
        ...
        match &input.fields {
            syn::Fields::Named(fields) => {
                ...
                if !has_output_attribute {
                    return Err(Error::new(
                        proc_macro2::Span::call_site(),
                        "MetaOutput structs must have at least one field with an #[output(description = \"...\")] attribute",
                    ));
                }
            }
            _ => {
...
mod tests {
    ...
    fn build_parser(input: DeriveInput) -> MetaOutputParser {
        let mut parser = MetaOutputParser::default();
    ...
    fn missing_output_attribute_errors() {
        ...
        assert!(
            err.to_string()
                .contains("MetaOutput structs must have at least one field")
        );
    }
